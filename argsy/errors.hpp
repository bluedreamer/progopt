...
public:
    ...
    *      the error template */
   void set_substitute(const std::string &parameter_name, const std::string &value) { m_substitutions[parameter_name] = value; }
...
public:
   ...
   /** Convenience functions for backwards compatibility */
   auto tokens() const -> std::string override { return m_substitutions.find("invalid_line")->second; };
};
...
{
public:
   invalid_command_line_syntax(kind_t kind, const std::string &option_name = "", const std::string &original_token = "",
                               int option_style = 0)
...
class BOOST_PROGRAM_OPTIONS_DECL invalid_option_value : public validation_error
{
public:
...
class BOOST_PROGRAM_OPTIONS_DECL invalid_bool_value : public validation_error
{
public:
